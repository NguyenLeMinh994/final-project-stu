  ...
  <div class="container">
    <div class="row no-gutters slider-text align-items-md-end align-items-center justify-content-end">
      <div class="col-md-6 text p-4 ftco-animate">
        ...
        <span class="location d-block mb-3"><i class="icon-my_location"> </i>Quận {{$silde->getSanPham->getQuan->ten}}
          , {{$silde->getSanPham->getTinhThanhPho->loai}} {{$silde->getSanPham->getTinhThanhPho->ten}}</span>
        <p>{{$silde->getSanPham->noidung}}</p>
        <span class="price">{{number_format($silde->getSanPham->gia)}} VNĐ</span>
        <a href={{ route("detail",["id"=>$silde['id']]) }} class="btn-custom p-3 px-4 bg-primary">Xem chi tiết <span
